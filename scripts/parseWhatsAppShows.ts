...
function parseTimestamp(ts: string): Date | null {
  ...
  return new Date(fullYear, parseInt(month) - 1, parseInt(day), parseInt(hour), parseInt(min), parseInt(sec));
}

// Parse day header: *Wednesday, August 21st* or *Wednesday* or *Friday, November 1*
function parseDayHeader(line: string): { dayOfWeek: string; dayNum?: number; month?: string } | null {
  ...
  const fullMatch = clean.match(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\s+(\w+)\s+(\d{1,2})(?:st|nd|rd|th)?$/i);
  if (fullMatch) {
    return {
  ...
  const reverseMatch = clean.match(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\s+(\d{1,2})(?:st|nd|rd|th)?\s+(\w+)$/i);
  if (reverseMatch) {
    return {
  ...
  const shortMatch = clean.match(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),?\s+(\d{1,2})\s+(\w+)$/i);
  if (shortMatch) {
    return {
...
function parseShowLine(line: string): { time: string; title: string; price?: string; hostedBy?: string } | null {
  let time = '';
  let title = '';
  ...
  let match = clean.match(/^(\d{1,2}):(\d{2})\s*(?:pm|am)?\s*[-–]\s*\d{1,2}:\d{2}\s*(?:pm|am)?:\s*(.+)$/i);
  if (match) {
  ...
  if (!time) {
    match = clean.match(/^(.+?)\s*[-:]\s*(\d{1,2}):(\d{2})\s*(?:pm|am)?(?:\s*[-–]\s*\d{1,2}:\d{2}\s*(?:pm|am)?)?/i);
    if (match) {
...
function parseExport(text: string): Show[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const tsMatch = line.match(/^\[(\d{1,2}\/\d{1,2}\/\d{2},\s*\d{1,2}:\d{2}:\d{2})\]\s*(.+?):\s*(.*)$/);
    if (tsMatch) {
      const ts = `[${tsMatch[1]}]`;
