...
async function syncToDatabase(icalData: string, imageUrls: Map<string, string>) {
	try {
		...
		const veventBlocks = unfoldedData
			.split('BEGIN:VEVENT')
			.slice(1)
...
export const GET: RequestHandler = async () => {
	try {
		...
		const icalUrl =
			import.meta.env.VITE_PROXY_ICAL_URL ||
		...
		for (const [url, imageUrl] of imageUrls) {
			modifiedIcalData = modifiedIcalData.replace(`URL:${url}`, `URL:${url}\nIMAGE:${imageUrl}`);
		}
		...
		syncToDatabase(icalData, imageUrls).catch((e) =>
			console.error('Background DB sync failed:', e)
		);
