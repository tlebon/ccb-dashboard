<script lang="ts">
	...
	$: houseTeams = teams.filter(t => t.type === 'house');
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
						{#each houseTeams as team (team.id)}
							<a
								...
								class="group p-5 border-l-4 border-[var(--tw-neon-pink)]/60 bg-white/5 hover:bg-white/10 transition-all"
							>
				...
				<section class="mb-12">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
						{#each indieTeams as team (team.id)}
							<a
								href="/teams/{team.slug}"
								class="group flex flex-col gap-1 py-3 px-4 border-l-4 border-[var(--tw-electric-cyan)]/40
